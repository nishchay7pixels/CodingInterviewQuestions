//Need to be completed using binary search approach
...
public class PainterProblem2 {
    ...
    static int painterScheduler(int arr[],int start, int end, int k, int[][] record){
        ...
        for(int index=1;index<end;index++){
            best = Math.min(best,Math.max(painterScheduler(arr, start, end-index, k-1, record), sumPartition(end-index+1, end,arr)));
        }
    ...
    public static void main(String[] args) {
        int arr[] = {10 , 20,30,40};
        int k=2;
        int [][] record = new int[k][arr.length];
        System.out.print(painterScheduler(arr, 0, arr.length-1, k, record));
    }
}
/*
k = 3 SOL[10, 20, 30, 40]
